...
### Translate Issue
Some tooltips are stuck in English because the source strings are missing from the language files.<br>
Unless AMD adds them in a future update, they'll likely stay that way for now.
...
### 日本語言語ファイルの既存の問題 1
「出荷時にリセット」のボタンの幅が合わないことでテキストラベルが途切れる問題があります。<br>
とりあえず強引ですが、リセットとシンプルなテキストにしています。ツールチップには詳細な説明が表示されるので勘弁を...

### 日本語言語ファイルの既存の問題 2
日本語だけに限った話ではありませんが、一部ツールチップが訳せない状態です。
これはリソースのソーステキストが存在しないからです。AMD 側がファイルを更新しない限りは英語のままと思ってください。
